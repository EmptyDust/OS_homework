...
PCB	*ready = NOTHING, *pfend = NOTHING,*p =NOTHING,*stop = NOTHING,*stopend = NOTHING;		//å°±ç»ªéåï¼è¿ç¨æå¥ä½ç½®çåé
...
{
    if(stop == NOTHING){
        printf("\né»å¡éåä¸ºç©ºï¼è¯·è¿è¡å¶ä»æä½");
        return;
    }
    else{
        if(ready != NOTHING){
            //å¾è¡¥å¨
			pfend = pfend->link = stop;
        }
        else{
            //å¾è¡¥å¨
			ready = pfend = stop;
...
int main()
{
	...
	while(ready != NOTHING || stop != NOTHING)
	{
		...
		printf("\næié®æ·»å æ°è¿ç¨...sé®é»å¡è¿ç¨...wé®å¤éè¿ç¨\næå¶ä»ä»»æé®ç»§ç»­è¿è¡...");
		fflush(stdin);
		ch = getchar();
		if (ch == 'i' || ch == 'I')
			input();
